...
prompt_template = "Classify the following invoice line item description:\n\n\"{}\"\n\nKeywords: {}"

invoice_descriptions = [
    "full bottom fiberglass rebuild to repair large hole and chipping in bathtub",
    "in AS White (to match surround walls as close as possible)",
    "website hosting fee",
    "3D tour content-leasing/Hosting"
]


for invoice_description in invoice_descriptions:
    # Add double quotes to each keyword
    quoted_keywords = ['"{}"'.format(keyword) for keyword in keywords]

    prompt = prompt_template.format(invoice_description, ', '.join(quoted_keywords))
    key = ""
